...
export default async function DashboardPage() {
  ...
  return (
    <div className="p-8 max-w-4xl mx-auto w-full flex flex-col h-full overflow-y-auto">
      ...
      <div className="bg-dark-gray-16/30 border border-dim-gray/10 rounded-xl p-6 mb-10 shadow-sm backdrop-blur-sm">
        <h2 className="text-sm font-medium text-white mb-1.5">Ingest a repository</h2>
        <p className="text-xs text-dark-gray mb-5">Paste a GitHub URL or pick from your recent repos to begin indexing.</p>
      ...
      <div className="pb-12">
        <div className="flex items-center justify-between mb-4 px-1">
          <h2 className="text-sm font-medium text-white">Recent Repositories</h2>
          <span className="text-xs text-dark-gray">{repos?.length ?? 0} total</span>
        </div>

        {!repos || repos.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-16 text-center border border-dashed border-dim-gray/20 rounded-xl bg-dark-gray-16/20">
            ...
            <p className="text-xs text-dark-gray/50">Paste a GitHub URL above to get started</p>
          </div>
        ) : (
          <div className="space-y-2">
            {repos.map((repo) => (
              <Link key={repo.id} href={`/repos/${repo.id}`} className="block">
                <div className="flex items-center gap-4 px-4 py-3.5 rounded-xl bg-dark-gray-16/30 border border-dim-gray/10 hover:border-dim-gray/30 hover:bg-dark-gray-16 group transition-all">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="text-[11px] text-dark-gray/60 mt-0.5 font-medium uppercase tracking-wider">
